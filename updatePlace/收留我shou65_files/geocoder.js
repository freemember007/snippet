...
(function() {
	...
	this.GeoCoder = GeoCoder = (function() {
		...
		GeoCoder._parseData = function(data) {
			...
			// 新接口返回值有变
			result.latitude = data.geometry.location.nb;
			result.longitude = data.geometry.location.ob;
			return result;
		};
		...
		GeoCoder.search = function(keyword, cb) {
			...
			if (keyword.indexOf(',') !== -1) {
				...
				keywordType = 'latLng';
				keywordValue = new google.maps.LatLng(lat, lng);
			} else {
				keywordType = 'address';
				keywordValue = keyword;
			}
			queryObj[keywordType] = keywordValue;
			var geocoder = new google.maps.Geocoder();
			if (geocoder) {
				geocoder.geocode(queryObj, function(results, status) {
					if (status == google.maps.GeocoderStatus.OK) {
						var result = _this._parseData(results[0]);
						return cb(result);
					} else {
						return cb(results);
					}
